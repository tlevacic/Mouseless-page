(ns site.forms.validators
  (:require [clojure.string :as str]
            [forms.validator :as v]
            [oops.core :refer [ocall]]
            [com.rpl.specter :as s :refer [recursive-path if-path]]))
...
(def map-key-walker
  (recursive-path [akey] p
    (s/cond-path map? [s/ALL (if-path [s/FIRST #(= % akey)] s/LAST [s/LAST p])]
                 vector? [s/ALL p])))



(defn unique-email [v data _]
  (let [emails (s/select (map-key-walker :email) data)]
    (if (> (get (frequencies emails) v) 1) false true)))

(def default-validations
  {:not-empty          {:message   "Value can't be empty"
                        :validator not-empty?}
   :true               {:message   "Must be selected"
                        :validator #(true? %1)}
   ...
   :email-same {:message   "Email cannot be same as old email"
                            :validator (fn [_ data _]
                                         (let [email              (:email data)
   ...
   :unique-email {:message "Every user must have an unique email"
